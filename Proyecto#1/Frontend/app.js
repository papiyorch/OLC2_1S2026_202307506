...
// Estado global
// ──────────────────────────────────────────────
const BACKEND_URL = 'http://localhost:8080/index.php';
...
function renderSymbolsTable(symbols) {
  ...
  symbols.forEach(s => {
    const val = s.value === null       ? 'nil'
              : typeof s.value === 'boolean' ? (s.value ? 'true' : 'false')
...
function downloadOutput() {
  download('resultado.txt', lastOutput || '(sin salida)', 'text/plain');
}
...
function downloadSymbols() {
  const header = 'Identificador,Tipo,Ámbito,Valor,Línea,Columna\n';
